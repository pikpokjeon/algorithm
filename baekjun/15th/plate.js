...
// 재석이는 대문에 붙이는 (주소를 나타내는) 호수판 제작업체의 직원이다. 고객에게 전달할 호수판은 숫자와 숫자 사이 
// 그리고 왼쪽 오른쪽으로 적당히 여백이 들어가 줘야하고 숫자마다 차지하는 간격이 조금씩 상이하다. 다행이도 규칙은 매우 간단하다. 
...
while(input.length)
{
   let num = input.shift().split('').map(e => Number(e))
   if(num[0] === 0) return
   // 숫자개수 -1 만큼 숫자사이 여백이 들어간다.
   // 1은 2, 2는3, 0은 4센티이다. 나머지 숫자는 3센티이다.
   // 호수판 경계에 1센티 여백이 들어간다
   let width = (num.length - 1) + 2
   num.forEach( e => 
      {
         if( e === 1) width += 2
         else if( e === 2) width += 3
